...
/** The recurrence kind for RFC 5545 recurrence rules, evaluated by rrule-temporal. */
export const RRULE_KIND = 'rrule'
 ...
 * to be the same one in every process and every release: a parser is called on whichever instance
 * runs the pass, and an anchor of "now" would move the phase of `INTERVAL=2` on every restart.
 *
...
function toIcs (expression: string): string {
  ...
  if (!dtstart) {
    ...
    // rule parses, has no occurrence left, and the schedule it lands on never fires again. Rejected
    // here because a schedule that silently does nothing is the one failure a caller cannot see.
...
/**
 * RFC 5545 recurrence rules, evaluated by rrule-temporal.
 *
 * The zone comes from DTSTART when it names one, and from the schedule's `tz` otherwise, which is
 * both what RFC 5545 says and what lets a rule written without a DTSTART behave like a cron
 * expression on the same schedule.
 */
export const rruleRecurrence: types.RecurrenceParser = {
  next (expression, after, tz) {
    const occurrence = buildRule(expression, tz).next(after)

    // A cron pass compares occurrences against a Date and stores them in a timestamptz, so the
    // nanoseconds a Temporal instant carries have nowhere to go. Nothing is lost: an iCalendar
  ...
  validate (expression, tz) {
    ...
    try {
      ...
      // The expression has already parsed once, so the zone is the only thing left to blame.
      // Reported in the same words the cron kind uses for the same mistake, rather than as the
      // uppercased "Invalid time zone specified" a Temporal implementation throws.
