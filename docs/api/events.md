...
| `transactional_pool_headroom` | A [`transactional`](./workers.md#workname-options-handler) worker was registered on a pool with no connections to spare. Each handler in flight holds one for its own duration, so at this size the fetches, the failures, and maintenance have nothing left to run on and wait out `connectionTimeoutMillis` instead. Emitted from `work()`, once per call | `max`, `transactionalWorkers` |
