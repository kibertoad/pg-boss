...
The table records episodes: a condition the database entered and can leave again. `transactional_pool_headroom` is the one type left out of it, because it reports a fixed relationship between `max` and the workers you registered, which no restart or later pass resolves. It is emitted as an event and nothing else.
